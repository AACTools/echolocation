...
| Screen | Main screen shows the pressed key immediately and keeps it after release; `text=` overrides replace the key label; green tick appears to the right after a hold-to-send key is sent; Debug overrides (off by default) shows the keys.txt token above the key and the yellow override summary below it |
